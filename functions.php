...
function coolmat_scripts() {
	//wp_enqueue_style looks for a default style.css file in our file directory

	wp_enqueue_style( 'coolmat-style', get_stylesheet_uri(), array(), _S_VERSION );

	// here we load our custom css file usiing wp_enqueue_style tag
...
// Register Custom Post Type - Intro
...
function get_category_description($query) {
	query_posts($query . '&post_per_page=1');
	while ( have_posts() ) {
		the_post();
